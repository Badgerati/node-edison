...
function Edison(global_opts) {
    // suppress console.log
    if (global_opts.disable_console_output) {
    ...
    // test results
    var _results = new Results(global_opts.console_output_type);
    ...
    Edison.prototype.test = function(name, opts, callback) {
        ...
        // if option set to skip/ignore, return
        if (args.opts.skip || args.opts.ignore) {
            return;
        }

        // check if this test's categories has a category to exclude, if so return
        if (Checkers.containsOneOf(args.opts.categories, global_opts.exclude)) {
            return;
        }

        // check if this test's categories is one of the included ones, if not return
        if (!Checkers.isEmpty(global_opts.include) && !Checkers.containsOneOf(args.opts.categories, global_opts.include)) {
            return;
        }
    ...
    function runTestFunc(setupError, setupEach, test, afterEach, done) {
        ...
        if (setupError) {
            ...
            _results.log(result, global_opts);
            done();
            return;
        }
...
Edison.create = function(opts) {
    ...
    // else, we need to set one up
        ...
        var _opts = {
            ...
            include: (gopts.include || opts.include),
            exclude: (gopts.exclude || opts.exclude)
        };
